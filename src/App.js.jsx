...
const T = {
  ...
  header: "#ffffff", navActive: "#eff6ff", navActiveBorder: "#bfdbfe",
  navActiveText: "#1d4ed8", input: "#f1f5f9", inputBorder: "#cbd5e1",
  overlay: "rgba(15,23,42,0.5)",
  blue: "#3b82f6", indigo: "#6366f1", green: "#10b981", red: "#ef4444", amber: "#f59e0b",
};
...
function Modal({ onClose, children, w = 460 }) {
  return (
    <div style={{position:"fixed",inset:0,background:T.overlay,display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,backdropFilter:"blur(4px)"}} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:28,width:w,boxShadow:"0 20px 60px rgba(0,0,0,0.15)",maxHeight:"88vh",overflowY:"auto"}}>
        {children}
      </div>
    </div>
  );
...
// ── LOGIN ──────────────────────────────────────────────────────
function LoginPage({ users, onLogin, onResetPassword }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f1f5f9",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Sarabun',sans-serif"}}>
      ...
      <div style={{width:400}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{width:64,height:64,background:"linear-gradient(135deg,#3b82f6,#6366f1)",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,margin:"0 auto 14px",boxShadow:"0 8px 24px rgba(59,130,246,0.3)"}}>⚙️</div>
          <div style={{fontSize:26,fontWeight:800,color:"#1e293b",fontFamily:"'Space Mono',monospace",letterSpacing:4}}>CPU</div>
          <div style={{fontSize:12,color:T.sub,marginTop:4}}>ระบบบริหารคลังสินค้า</div>
        </div>

        <div style={{background:"white",border:`1px solid ${T.border}`,borderRadius:16,padding:32,boxShadow:"0 4px 20px rgba(0,0,0,0.08)"}}>
          <div style={{fontSize:16,fontWeight:700,color:T.text,marginBottom:20}}>เข้าสู่ระบบ</div>
          ...
          <button onClick={handle} disabled={loading} style={{width:"100%",padding:"11px",background:"linear-gradient(135deg,#3b82f6,#6366f1)",color:"white",border:"none",borderRadius:10,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'Sarabun',sans-serif",opacity:loading?0.7:1,boxShadow:"0 4px 14px rgba(59,130,246,0.35)"}}>
            {loading?"กำลังเข้าสู่ระบบ...":"เข้าสู่ระบบ"}
          </button>
          <div style={{marginTop:18,padding:14,background:"#f8fafc",borderRadius:10,border:`1px solid ${T.border}`}}>
            <div style={{fontSize:11,color:T.muted,marginBottom:8}}>🔑 บัญชีทดสอบ (รหัสผ่าน: 1234)</div>
            {users.map(u=>(<div key={u.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:T.sub,marginBottom:4}}><span>{u.avatar} <b style={{color:T.text}}>{u.username}</b></span><span style={{color:ROLES[u.role].color,fontSize:11,fontWeight:600}}>{ROLES[u.role].label}</span></div>))}
          </div>
        </div>
      </div>
      ...
      {showForgot&&(
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,backdropFilter:"blur(4px)"}} onClick={closeForgot}>
          <div onClick={e=>e.stopPropagation()} style={{background:"white",border:`1px solid ${T.border}`,borderRadius:16,padding:28,width:420,boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
            {/* Header */}
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              <div>
                <div style={{fontSize:15,fontWeight:700,color:T.blue}}>🔑 ลืมรหัสผ่าน</div>
            ...
            <div style={{display:"flex",gap:6,marginBottom:20}}>
              {[1,2,3].map(s=>(
                <div key={s} style={{flex:1,height:4,borderRadius:2,background:forgotStep>=s?"#3b82f6":"#e2e8f0",transition:"background .3s"}}/>
              ))}
            </div>
...
export default function App() {
  ...
  // forms
  const [newProduct, setNewProduct] = useState({ code:"",name:"",category:"",qty:"",unit:"",minQty:"",location:"",barcode:"",image:"" });
  ...
  if (loading) return (
    <div style={{minHeight:"100vh",background:"#f1f5f9",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Sarabun',sans-serif"}}>
      ...
      <div style={{width:64,height:64,background:"linear-gradient(135deg,#3b82f6,#6366f1)",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,marginBottom:20,boxShadow:"0 8px 24px rgba(59,130,246,0.3)"}}>⚙️</div>
      <div style={{fontSize:20,fontWeight:800,color:"#1e293b",fontFamily:"'Space Mono',monospace",letterSpacing:4,marginBottom:8}}>CPU</div>
      <div style={{width:32,height:32,border:"3px solid #bfdbfe",borderTop:"3px solid #3b82f6",borderRadius:"50%",animation:"spin 0.8s linear infinite",marginTop:16}}/>
      <div style={{fontSize:12,color:"#64748b",marginTop:12}}>กำลังเชื่อมต่อฐานข้อมูล...</div>
    </div>
  );

  if (!user) return <LoginPage users={users} onLogin={u => { setUser(u); setProfileForm({name:u.name,username:u.username,oldPass:"",newPass:"",confirmPass:""}); }} onResetPassword={handleResetPassword}/>;
  ...
  const BtnPrimary = ({onClick,children,disabled=false,style={}}) => (
    <button onClick={onClick} disabled={disabled} style={{padding:"8px 18px",borderRadius:8,border:"none",cursor:disabled?"not-allowed":"pointer",background:"linear-gradient(135deg,#3b82f6,#6366f1)",color:"white",fontSize:13,fontWeight:600,fontFamily:"'Sarabun',sans-serif",opacity:disabled?0.45:1,...style}}>{children}</button>
  );
  const BtnSuccess = ({onClick,children,disabled=false,style={}}) => (
    <button onClick={onClick} disabled={disabled} style={{padding:"8px 18px",borderRadius:8,border:"none",cursor:disabled?"not-allowed":"pointer",background:"linear-gradient(135deg,#059669,#10b981)",color:"white",fontSize:13,fontWeight:600,fontFamily:"'Sarabun',sans-serif",opacity:disabled?0.45:1,...style}}>{children}</button>
  );
  const BtnDanger = ({onClick,children,disabled=false,style={}}) => (
    <button onClick={onClick} disabled={disabled} style={{padding:"8px 18px",borderRadius:8,border:"none",cursor:disabled?"not-allowed":"pointer",background:"linear-gradient(135deg,#dc2626,#ef4444)",color:"white",fontSize:13,fontWeight:600,fontFamily:"'Sarabun',sans-serif",opacity:disabled?0.45:1,...style}}>{children}</button>
  );
  const BtnGhost = ({onClick,children,style={}}) => (
    <button onClick={onClick} style={{padding:"8px 18px",borderRadius:8,border:`1px solid ${T.border}`,cursor:"pointer",background:"transparent",color:T.sub,fontSize:13,fontWeight:500,fontFamily:"'Sarabun',sans-serif",...style}}>{children}</button>
  );
  ...
  const CardBox = ({children, style={}}) => (
    <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:14,padding:20,boxShadow:"0 1px 4px rgba(0,0,0,0.04)",...style}}>{children}</div>
  );
    ...
    <div style={{display:"flex",height:"100vh",fontFamily:"'Sarabun',sans-serif",background:T.bg,color:T.text,overflow:"hidden"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        input:focus{outline:none;border-color:#93c5fd !important;box-shadow:0 0 0 3px rgba(147,197,253,0.2)}
      ...
      <div style={{width:sidebarOpen?224:60,background:T.sidebar,borderRight:`1px solid ${T.border}`,transition:"width .28s",display:"flex",flexDirection:"column",flexShrink:0,boxShadow:"2px 0 8px rgba(0,0,0,0.04)"}}>
        <div style={{padding:"18px 16px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:34,height:34,background:"linear-gradient(135deg,#3b82f6,#6366f1)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:18,boxShadow:"0 2px 8px rgba(59,130,246,0.35)"}}>⚙️</div>
        ...
        <nav style={{padding:"10px 8px",flex:1}}>
          {navItems.map(item => {
            ...
            return (
              <div key={item.id} onClick={() => setActiveTab(item.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 14px",borderRadius:10,cursor:"pointer",transition:"all .18s",color:active?T.navActiveText:T.sub,fontWeight:active?600:400,fontSize:14,background:active?T.navActive:"transparent",border:active?`1px solid ${T.navActiveBorder}`:"1px solid transparent",marginBottom:2,justifyContent:sidebarOpen?"flex-start":"center"}}>
        ...
        {sidebarOpen && (
          <div style={{padding:"12px 16px",borderTop:`1px solid ${T.border}`}}>
            ...
            <button onClick={() => setUser(null)} style={{width:"100%",padding:"7px",borderRadius:8,border:`1px solid ${T.border}`,background:"transparent",color:T.red,fontSize:12,cursor:"pointer",fontFamily:"'Sarabun',sans-serif",fontWeight:500}}>🚪 ออกจากระบบ</button>
          </div>
        )}
      ...
      <div style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"13px 24px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",background:T.header,position:"sticky",top:0,zIndex:10,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {activeTab==="inventory"&&role.canManageCats&&<BtnGhost onClick={()=>setShowCatModal(true)}>📦 หมวดหมู่</BtnGhost>}
            {activeTab==="inventory"&&role.canAdd&&<BtnPrimary onClick={()=>setShowAddModal(true)}>＋ เพิ่มสินค้า</BtnPrimary>}
            {(activeTab==="inventory"||activeTab==="transactions")&&<>
            ...
            {activeTab==="inventory"&&role.canClear&&<button onClick={()=>setShowClearConfirm(true)} style={{padding:"8px 16px",borderRadius:8,border:`1px solid #fca5a5`,background:"#fef2f2",color:T.red,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'Sarabun',sans-serif"}}>🗑 ล้างคลัง</button>}
          </div>
        </div>
        ...
        <div style={{padding:24,flex:1}}>
          ...
          {activeTab==="dashboard"&&(
            <div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:24}}>
                {[
                  {label:"สินค้าทั้งหมด",value:products.length,unit:"รายการ",icon:"📦",color:T.blue,bg:"#eff6ff"},
                  {label:"จำนวนรวม",value:totalQty.toLocaleString(),unit:"หน่วย",icon:"📊",color:T.green,bg:"#f0fdf4"},
                  {label:"ต่ำกว่าขั้นต่ำ",value:lowStock.length,unit:"รายการ",icon:"⚠️",color:T.red,bg:"#fef2f2"},
                  {label:"รายการเคลื่อนไหว",value:transactions.length,unit:"ครั้ง",icon:"🔄",color:T.amber,bg:"#fffbeb"},
                ].map((s,i)=>(
                  <div key={i} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:14,padding:20,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
                      <div style={{fontSize:11,color:T.sub,fontWeight:500}}>{s.label}</div>
                      <div style={{width:34,height:34,borderRadius:8,background:s.bg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{s.icon}</div>
                    </div>
                    <div style={{fontSize:30,fontWeight:700,color:s.color,fontFamily:"'Space Mono',monospace"}}>{s.value}</div>
                    <div style={{fontSize:11,color:T.muted,marginTop:2}}>{s.unit}</div>
                  </div>
                ))}
              </div>
          ...
          {activeTab==="inventory"&&(
            <div>
              ...
              <CardBox style={{padding:0,overflow:"hidden"}}>
                {/* Table header */}
                <div style={{display:"grid",gridTemplateColumns:"44px 90px 1fr 110px 70px 70px 110px 80px 100px",alignItems:"center",padding:"10px 16px",background:"#f8fafc",borderBottom:`1px solid ${T.border}`,color:T.muted,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em"}}>
                ...
                {filtered.length===0?(
                  ...
                  <div key={p.id} style={{display:"grid",gridTemplateColumns:"44px 90px 1fr 110px 70px 70px 110px 80px 100px",alignItems:"center",padding:"11px 16px",borderBottom:i<filtered.length-1?`1px solid ${T.border}`:"none",transition:"background .15s"}}
                    ...
                    <div>
                      {p.image
                        ...
                        :<div style={{width:38,height:38,borderRadius:6,background:"#f1f5f9",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,border:`1px dashed ${T.inputBorder}`,cursor:"pointer"}} onClick={()=>setShowImgModal(p)}>📷</div>
                      }
                    </div>
          ...
          {activeTab==="transactions"&&(
            <CardBox style={{padding:0,overflow:"hidden"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"70px 80px 1fr 80px 160px 170px",alignItems:"center",padding:"10px 16px",background:"#f8fafc",borderBottom:`1px solid ${T.border}`,color:T.muted,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em"}}>
          ...
          {activeTab==="users"&&role.canManageUsers&&(
            <div style={{maxWidth:640}}>
              <CardBox>
                <div style={{fontSize:14,fontWeight:600,color:T.text,marginBottom:16}}>👥 รายชื่อผู้ใช้งาน</div>
                {users.map(u=>(
                  <div key={u.id} style={{display:"flex",alignItems:"center",gap:14,padding:"12px 0",borderBottom:`1px solid ${T.border}`}}>
                    ...
                    <div style={{flex:1}}><div style={{fontWeight:600,color:T.text}}>{u.name}</div><div style={{fontSize:12,color:T.sub}}>@{u.username}</div></div>
                    <Badge bg={`${ROLES[u.role].color}15`} color={ROLES[u.role].color}>{ROLES[u.role].label}</Badge>
                    <div style={{fontSize:11,color:T.muted}}>{ROLES[u.role].canAdd?"✅":"❌"} เพิ่ม · {ROLES[u.role].canDelete?"✅":"❌"} ลบ · {ROLES[u.role].canClear?"✅":"❌"} ล้างคลัง</div>
                  </div>
                ))}
              </CardBox>
            </div>
      ...
      {showAddModal&&(
        <Modal onClose={()=>setShowAddModal(false)} w={520}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            {[{k:"code",l:"รหัสสินค้า *",ph:"เช่น RM-004"},{k:"name",l:"ชื่อสินค้า *",ph:"ชื่อสินค้า"},{k:"unit",l:"หน่วยนับ *",ph:"เช่น ชิ้น, กล่อง"},{k:"location",l:"ตำแหน่งที่เก็บ",ph:"เช่น A-05"},{k:"qty",l:"จำนวนเริ่มต้น *",ph:"0",t:"number"},{k:"minQty",l:"จำนวนขั้นต่ำ",ph:"0",t:"number"}].map(f=>(
      ...
      {showCatModal&&(
        <Modal onClose={()=>setShowCatModal(false)} w={420}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16,padding:12,background:"#f8fafc",borderRadius:10,border:`1px solid ${T.border}`,minHeight:48}}>
            {categories.map(c=>(
              <span key={c} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:20,fontSize:12,fontWeight:500,background:T.navActive,color:T.blue,border:`1px solid ${T.navActiveBorder}`}}>
                {c}
      ...
      {/* ── MODAL: ดูรูป ── */}
      {showImgModal&&(
        <Modal onClose={()=>setShowImgModal(null)} w={400}>
          <MHead title={showImgModal.name} sub={showImgModal.code} onClose={()=>setShowImgModal(null)}/>
          {showImgModal.image
            ?<img src={showImgModal.image} alt="" style={{width:"100%",maxHeight:320,objectFit:"contain",borderRadius:10,marginBottom:16}}/>
            :<div style={{height:160,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:T.muted,marginBottom:16}}><div style={{fontSize:48,marginBottom:8}}>📷</div><div style={{fontSize:13}}>ยังไม่มีรูปสินค้า</div></div>
          }
      ...
      {showSettings&&(
        <Modal onClose={()=>setShowSettings(false)} w={520}>
          ...
          {settingsTab==="system"&&(
            <div>
              <div style={{fontSize:13,fontWeight:600,color:T.text,marginBottom:14}}>ข้อมูลระบบ</div>
              {[
                {label:"จำนวนสินค้าในคลัง",value:`${products.length} รายการ`},
                {label:"สินค้าต่ำกว่าขั้นต่ำ",value:`${lowStock.length} รายการ`},
                {label:"รายการเคลื่อนไหวทั้งหมด",value:`${transactions.length} รายการ`},
                {label:"จำนวนหมวดหมู่",value:`${categories.length} หมวดหมู่`},
                {label:"จำนวนผู้ใช้งาน",value:`${users.length} บัญชี`},
              ].map((r,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"11px 0",borderBottom:i<4?`1px solid ${T.border}`:"none"}}>
                  <span style={{fontSize:13,color:T.sub}}>{r.label}</span>
                  <span style={{fontSize:13,fontWeight:600,color:T.text}}>{r.value}</span>
                </div>
              ))}
              {role.canClear&&(
                <div style={{marginTop:20,padding:16,background:"#fef2f2",border:"1px solid #fecaca",borderRadius:10}}>
          ...
          {settingsTab==="about"&&(
            <div style={{textAlign:"center",padding:"10px 0"}}>
              <div style={{width:64,height:64,background:"linear-gradient(135deg,#3b82f6,#6366f1)",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:30,margin:"0 auto 16px",boxShadow:"0 8px 24px rgba(59,130,246,0.3)"}}>⚙️</div>
